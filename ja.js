const ja = {
  ...
  tourGuide: 
  {
    ...
    description_search_bar: "名前を入力して検索ボタンをタップして,既存のリストでオブジェクトをフィルタリングするか,新しいオブジェクトを検索します。",
    description_time_controls: "Ekos がオフラインの場合は,対象の日時の計算を調整します。",
    description_target_controls: "確認トワイライト情報,FOV の管理,フィルターの調整,天体の種類の選択。",
  ...
  status: 
  {
    ...
    alert_resize_body: " StellarMateはSDカードのフルサイズに拡張されます.サイズ変更後にStellarMateを再起動してください.",
    alert_resize_btn_resize: "サイズ変更",

    alert_hotspot_enable_title: "ホットスポットを有効にしています",
    alert_hotspot_enable_body: "ホットスポットを有効にすると,stellarmate は現在参加している Wi-Fi ネットワークから切断され,独自のホットスポットを開始します。StellarMate を引き続き使用するには,デバイスから同じホットスポット ネットワークに接続する必要があります。続行するには?",
    ...
    alert_hotspot_disable_body: "ホットスポットを無効にすると、StellarMate は以前に記憶されたワイヤレス ネットワークに接続されます。ホットスポットが無効になっている場合、StellarMate を引き続き使用するには、同じネットワーク上にいる必要があります。続行しますか?",
    ...
    alert_forget_wifi_body: "すべての WiFi ネットワークが消去され,StellarMate はホットスポット モードで実行されます。携帯電話/タブレットでホットスポット ネットワークに切り替える必要があります。続けたいですか？",

    alert_join_wifi_title: "Wi-Fi ネットワークに参加",
    alert_join_wifi_body: "Wi-Fi ネットワークに参加すると,StellarMate によるホットスポット (開始されている場合) が閉じます。StellarMate を引き続き使用するには,同じネットワーク上にいることを確認してください。続行しますか?",
    alert_change_band_title: "帯域を変更してもよろしいですか?",
    alert_change_band_body: "StellarMate アプリが切断され,チャネルの切り替えがさまざまな理由で失敗する可能性があります。その場合,ホットスポットはデフォルト設定に復元されます.",
